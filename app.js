...
(function(){
  ...
  const form=$('#orderForm'), btn=$('#submitBtn');
  
  async function runSeq(){
    if(btn.classList.contains('active')) return;
    
    // Добавляем класс active для запуска CSS-анимаций
    btn.classList.add('active');
    
    // Ждем завершения всей анимации
    await wait(5500);
    
    // Убираем класс active для сброса анимации
  ...
  form.addEventListener('submit', async e=>{
    ...
    // После завершения анимации сбрасываем форму
    const u=getSaved()||{};
    form.reset();
    $('#date').value=todayISO();
    $('#name').value = (u.type==='company' ? (u.company?.contact||u.name||'') : (u.name||''));
